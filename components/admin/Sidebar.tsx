...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:sticky top-0 left-0 z-[110] rounded-r-4xl bg-white text-slate-800 flex h-screen flex-col border-r border-slate-200 shadow-xl transition-all duration-300 ease-in-out ${mobileOpen
            ? "translate-x-0"
            : "-translate-x-full lg:translate-x-0"
          } ${collapsed
            ? "w-20 overflow-visible"
            : "w-[85vw] max-w-72 lg:w-72 overflow-y-auto"
          }`}
      >
        {/* ===== HEADER ===== */}
        <div
          className={`h-18 border-b border-violet-100 bg-white flex items-center ${collapsed ? "justify-center px-2" : "justify-between px-4"
            }`}
        >
          {!collapsed && (
        ...
        {/* ===== NAVIGATION ===== */}
        <nav
          className={`flex-1 space-y-2 bg-white px-3 py-6 no-scrollbar ${collapsed ? "overflow-visible" : ""
            }`}
        >
          {navItems.map((item) => {
            const isActive =
        ...
        <div className="border-t border-violet-100 bg-white p-3">
          {/* ===== LOGOUT BUTTON ===== */}
          <div
            className={`overflow-hidden transition-all pb-4 duration-300 ease-in-out ${showLogout
          ...
          >
            <button
              onClick={() =>
                onLogoutRequest ? onLogoutRequest() : undefined
              }
